...
public class ParticleTests
{
    ...
    // ---- the drift is in screen space, not the art's -----------------------------------------------

    [Fact]
    public void AnUprightUnscaledArtNeedsNoCorrection()
    {
        foreach (var v in AllVariations)
        {
            var screen = AuraParticles.MotionFor(v).Direction;

            Assert.Equal(screen, AuraParticles.LocalHeading(screen, Transform2D.Identity));
        }
    }
    ...
    [InlineData(-90)]
    [InlineData(180)]
    public void ARotatedArtDoesNotRotateTheDrift(int degrees)
    {
        // NHandImage._Ready rotates each player's arm by 0, +/-90 or 180 so it reaches in from a different
        // side. Left-to-right has to stay left-to-right ON SCREEN, or the key means a different thing for
        // each player — which is worse than no key at all.
        var art = Transform2D.Identity.Rotated(Mathf.DegToRad(degrees));

        foreach (var v in AllVariations)
        {
            var screen = AuraParticles.MotionFor(v).Direction;
            var asDrawn = art.BasisXform(AuraParticles.LocalHeading(screen, art)).Normalized();

            Assert.Equal(screen.X, asDrawn.X, 4);
            Assert.Equal(screen.Y, asDrawn.Y, 4);
        }
    }

    [Fact]
    public void AMirroredArtDoesNotMirrorTheDrift()
    {
        // NRestSiteCharacter.FlipX negates the spine node's Scale.X, which would swap brighter and darker
        // for whichever players are sitting the other way round the campfire.
        var art = Transform2D.Identity.Scaled(new Vector2(-1f, 1f));
        var screen = AuraParticles.MotionFor(PlayerVariation.Brighter).Direction;
    ...
    [Fact]
    public void ScalingTheArtDoesNotBendTheDrift()
    {
        var art = Transform2D.Identity.Scaled(new Vector2(0.28f, 0.28f)).Rotated(0.7f);
        var screen = AuraParticles.MotionFor(PlayerVariation.Cooler).Direction;

        var asDrawn = art.BasisXform(AuraParticles.LocalHeading(screen, art)).Normalized();

        Assert.Equal(screen.X, asDrawn.X, 3);
        Assert.Equal(screen.Y, asDrawn.Y, 3);
    }

    [Fact]
    public void ADegenerateTransformLeavesTheHeadingAlone()
    {
        // A collapsed basis has no inverse. Better the uncorrected heading than a NaN, which Godot would
        // turn into particles that never move or never appear.
        var collapsed = new Transform2D(Vector2.Zero, Vector2.Zero, Vector2.Zero);
        var screen = AuraParticles.MotionFor(PlayerVariation.Warmer).Direction;

        Assert.Equal(screen, AuraParticles.LocalHeading(screen, collapsed));
    }

    [Fact]
    public void MoteSizeAccountsForTheArtsOwnScale()
    {
        // With LocalCoords off, Godot transforms a particle's spawn position and velocity by the emitter's
        // transform but NOT its scale amount — that is left in canvas units. A SpineSprite is scaled around
        // 0.28, so a mote sized against a radius measured in its local units would come out several times
        // too large without this.
        var big = AuraParticles.ScaleFor(400f, 32f, globalScale: 0.28f);
        var small = AuraParticles.ScaleFor(112f, 32f, globalScale: 1f);
    ...
    public void TheSpawnCloudIsSymmetric()
    {
        ...
        var quadrants = new[]
        {
            points.Count(p => p.X >= 0 && p.Y >= 0),
            points.Count(p => p.X < 0 && p.Y >= 0),
            points.Count(p => p.X >= 0 && p.Y < 0),
            points.Count(p => p.X < 0 && p.Y < 0),
        };

        Assert.True(quadrants.Max() < 1.2f * quadrants.Min(), "the cloud leans to one side");
    }
    ...
    [Fact]
    public void TheSpawnCloudIsTheSameOnEveryClient()
    {
    ...
    [Fact]
    public void TheSpawnCloudNeverProducesAnUnplaceableMote()
    {
        Assert.All(Cloud(), p =>
        {
            Assert.True(float.IsFinite(p.X) && float.IsFinite(p.Y), $"non-finite spawn point {p}");
        });
    }
    ...
    {
        // One key, two ways of reading it. A mote in a different colour from the glow it drifts through
        // would split the signal in half.
        foreach (var v in AllVariations)
        {
            var aura = PlayerTint.AuraColor(v);
            var mote = AuraParticles.ColorFor(v, 1f);

            Assert.Equal(aura.R, mote.R, 4);
            Assert.Equal(aura.G, mote.G, 4);
            Assert.Equal(aura.B, mote.B, 4);
        }
    }
    ...
    public void TheStrengthDialScalesEveryVariationsOwnOpacity()
    {
        ...
        foreach (var v in AllVariations)
        {
            ...
            Assert.Equal(own, AuraParticles.ColorFor(v, 1f).A, 4);
            Assert.Equal(own * 0.5f, AuraParticles.ColorFor(v, 0.5f).A, 4);
            Assert.Equal(0f, AuraParticles.ColorFor(v, 0f).A, 4);
        }
    }
    ...
    public void TheStrengthDialIsClamped(float requested, float expected)
    {
        Assert.Equal(expected, PlayerTint.ClampParticleStrength(requested), 4);
    }

    [Fact]
    public void TheStrengthDialDefaultsToLeavingTheTunedOpacitiesAlone()
    {
        Assert.Equal(1f, PlayerTint.DefaultParticleStrength, 4);
    }
    ...
    [InlineData(0, 0)]
    [InlineData(120, 120)]
    [InlineData(99999, PlayerTint.MaxParticleCount)]
    public void ParticleCountIsClamped(int requested, int expected)
    {
    ...
    public void ParticleSizeIsClamped(float requested, float expected)
    {
